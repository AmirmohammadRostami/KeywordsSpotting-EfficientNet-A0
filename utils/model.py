...
class SpeechEffModel(SerializableModule):
    def __init__(self, config):
        ...
        eff_n = config['base_model']
        self.model = EfficientNet.from_name(f'efficientnet-b{eff_n}',
                                            in_channels=1,
                                            **{'num_classes': n_labels,
                                               'width_coefficient': 0.3,
                                               'depth_coefficient': 0.42,
                                               'image_size': (201, 40),
...
class SpeechDataset(data.Dataset):
    ...
    def __init__(self, data, set_type, config):
        ...
        self.LENGTH = 2
        self.BOUND = 0.15
        self.VOICE_WINDOW = np.clip(librosa.filters.get_window(
            ('kaiser', 1.5), self.SR)+np.abs(np.random.normal(0, 0.1)), 0, 1.1)
        self.BACKGROUND_WINDOW = 1 - \
            librosa.filters.get_window(('kaiser', 2.5), self.SR*(1+2*self.PADD))+0.05
        self.audio_files = list(data.keys())
        self.set_type = set_type
        self.audio_labels = list(data.values())
        config["bg_noise_files"] = list(
            filter(lambda x: x.endswith("npy"), config.get("bg_noise_files", [])))
        ...
        self.labels_count = torch.tensor(max(labels_count)/labels_count)
    ...
    def load_audio(self, example, silence=False, lbl=0):
        ...
        bg_noise_multi = random.random() * 0.12
        ...
        try:
            ...
            if example != "__silence__" and self.set_type == DatasetType.TRAIN:
                data *= np.clip(self.VOICE_WINDOW[:len(data)] +
                                np.abs(np.random.normal(0, 0.02)), 0, 1.1)
                background = self._get_bg_voice()
                start_index = np.random.randint(
                    int(self.BOUND*self.SR), len(background)-len(self.BACKGROUND_WINDOW)-int(self.BOUND*self.SR))
                background[start_index:start_index +
                           len(self.BACKGROUND_WINDOW)] *= self.BACKGROUND_WINDOW
    ...
    def splits(cls, config):
        ...
        for folder_name in os.listdir(folder):
            ...
            if folder_name != "_background_noise_" and folder_name != "_background_" and folder_name in words:
                dev_data = list(np.random.choice(
                    np.arange(len(os.listdir(path_name))), dev_count, replace=False))
        ...
        test_cfg = ChainMap(dict(bg_noise_files=bg_noise_files,
                                 bg_files=bg_files, noise_prob=0), config)
...
_configs = {
    ...
    ConfigType.EFFA0.value: dict(n_labels=12, base_model=0),
    ConfigType.EFFA1.value: dict(n_labels=12, base_model=1),
    ConfigType.EFFA2.value: dict(n_labels=12, base_model=2),
}
